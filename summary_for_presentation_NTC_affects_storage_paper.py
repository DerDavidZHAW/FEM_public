...
# scenarios_display_names = scenarios_to_summarize
...
# Axes limits
added_power_max = 4.3  # GW
pit_tes_max = 670  # GWh

name_output = "investment_summary_for_presentation.csv"

rows = ["battery", "TTES", "PTES", "pvrf", "windon", "CCGT", "SCGT",  "dam", "electrolyzer", "hydrogen", "nuclear", "oil", "psp_close", "biomass", "other",  "V2G_", "fossilmethane", "resmethane", "CHPNew", "HPG", "HPNew",  "dsrTh", "resistive", "thermal"]
row_renaming = {
     "CCGT" : "CCGT",
     "SCGT" : "SCGT",
     "battery" : "Battery",
...
for scen in scenarios_to_summarize:
    data = pd.read_csv(f"output/{scen}/investment_summary.csv", index_col=0)
    P_allinv_raw = pd.read_csv(f"output/{scen}/P_allinv.csv",index_col=0)
    P_allinv = [tech for tech in P_allinv_raw.index]
    heating_techs = read_index_as_list_safe(f"output/{scen}/genTh_max.csv", index_col=0, usecols=[0, 2])
...
techs_to_plot_power = ["Battery", "additional PV", "Tank TES", "Pit TES", "Resistive Heater", "Heat pump"]
...
for annotation in [["el", -37, 1, added_power_max], ["th", -12, 1, added_power_max], ["el", -60, 2, pit_tes_max], ["th", -35, 2, pit_tes_max]]:
    fig.add_annotation(
        text=annotation[0],
        x="Tank TES",
        xshift=annotation[1],
...
# pio.write_image(
#     fig,
#     "investment_overview.pdf",  # output filename
#     format="pdf",
#     width=PLOT_WIDTH,
#     height=PLOT_HEIGHT,
...
pio.write_image(
    ...
    "investment_overview.pdf",       # Output filename
    format="pdf",
    # "investment_overview.png",
    # format="png",
    width=PLOT_WIDTH,
    height=PLOT_HEIGHT,
    scale=300 / (96/2)             # For high-resolution (300 DPI)
)