...
def aggregate_params_vars(scenarios_to_agg, item, agg_type_dict, output_dir, map_scen_subscen):
    ...
    # NOTE: David fixed a big that only occured with settings by implementing a new approach. However, it only works, if there is not a seperate column containing the scenario name. So for everything but the statistics and the settings, the old approach is used (where it used to work fine)
    if item in ['statistics', 'settings']:
        ...
        # read the first file to get the indices
        raw = pd.read_csv(filenames_paths_dict[scenarios_to_agg[0]][0],index_col=0)

        # Extract the indices
        indices = raw.index.to_list()

        # Check whether in this aggregation, there is one column per scenario or one column per subscenario
        if len(raw.columns) == 1:
            names = {scen: [scen] for scen in scenarios_to_agg}
        else:
            names = map_scen_subscen


        # Create a dataframe with the correct indices that will contain all the data
        ...
        for scenario_name, file_path_list in filenames_paths_dict.items():
            ...
            temp = pd.read_csv(file_path_list[0], index_col=0)

            # Check whether in this file, there is one column per scenario or one column per subscenario
            ...
            temp.columns = names[scenario_name]

            # insert the subscenarios into the data one by one
            for n in names[scenario_name]:
                data[n] = temp[n]
        ...
        if len(map_scen_subscen[scenario_name]) > 1:
            subscen_exists = True
        else:
...
def aggregate_indicators(scenarios_to_agg, item, agg_type, output_dir,map_scen_subscen):
    ...
    if item == "price_weighted":
        ...
        price_hourly.to_csv(output_dir + "prices_hourly.csv")
...
def op_inv_exp_imp_cost_calc(scenarios_to_agg, output_dir, map_scen_subscen):
    ...
    # lost load costs - CH ---------------------------------------------------------------------
    # read lost load values
    lost_load_hourly = pd.read_csv(
        output_dir + "lostload_hour_sum_temporal.csv")
    # keep only the rows where Consumer starts with CH0
    lost_load_hourly = lost_load_hourly.loc[lost_load_hourly["Consumer"].str.startswith("CH0")]

    # read lost load costs
    lost_load_costs_per_step = pd.read_csv(
        f"output/{scenarios_to_agg[0]}/lostload_cost_per_step.csv", index_col=0)
    
    # calculate the lost load costs for each scenario
    lost_load_costs = pd.DataFrame(columns=subcescen_list, index=lost_load_costs_per_step.index)
    for scenario in subcescen_list:
        total_cost_scenario = 0
        for step in lost_load_costs_per_step.index:
            # get cost for step
            cost_value = lost_load_costs_per_step.loc[step, "value"]

            if isinstance(cost_value, pd.Series):
                cost = cost_value.iloc[0]
            else:
                cost = cost_value
                
            # get lost load for step
            lost_load_step = lost_load_hourly.loc[lost_load_hourly["lostLoad_step"] == step, scenario].sum()
            total_cost_scenario += cost * lost_load_step #type:ignore
        lost_load_costs.loc[0, scenario] = total_cost_scenario
    
    data_to_plot.loc["lost load costs",:] = lost_load_costs.iloc[0]
...
def op_inv_exp_imp_cost_plot(scenarios_to_agg, map_scen_subscen, output_dir, file_name="op_inv_exp_imp_cost_plot"):
    ...
    positive_group = ["sum_price_multiplied_by_demand","investment costs", "lost load costs"]
    ...
    # Positive (costs) Group (Stacking)
    positive_group = ["investment costs", "operational costs", "lost load costs"]
    for variable in positive_group:  # "investment costs", "operational costs", "payment during import", "payment during export", 
        fig.add_trace(go.Bar(x=subcescen_list,
                            y=data_to_plot.loc[variable,:], 
                            name=variable,
